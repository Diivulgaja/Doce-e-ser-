  ...
  return (
    <div className="min-h-screen bg-[#fbf7f0] text-[#3c2419]">
      ...
      <main>
          ...
          <div className="relative mx-auto grid max-w-7xl items-center gap-10 px-4 py-12 sm:px-6 sm:py-16 lg:grid-cols-[1fr_.82fr] lg:py-20">
            <div>
              ...
              <div className="mt-7 flex max-w-2xl flex-wrap gap-2 text-xs font-semibold text-[#684430]">
                <span className="inline-flex items-center gap-1.5 rounded-full border border-[#a98264]/20 bg-white/60 px-3 py-2"><Check className="size-3.5 text-[#477444]" /> Retirada agendada</span>
                <span className="inline-flex items-center gap-1.5 rounded-full border border-[#a98264]/20 bg-white/60 px-3 py-2"><Clock3 className="size-3.5 text-[#8b5d3d]" /> Pagamento na retirada</span>
        ...
        <section id="loja" className="relative overflow-hidden bg-[#5b2c16] text-[#fff8ed]"><div className="absolute -right-24 -top-24 size-80 rounded-full bg-[#b87945]/15 blur-3xl" /><div className="relative mx-auto grid max-w-7xl gap-10 px-4 py-14 sm:px-6 md:grid-cols-2 md:py-20"><div><p className="text-sm font-semibold uppercase tracking-[.2em] text-[#e8bd80]">Retire com tranquilidade</p><h2 className="mt-3 max-w-lg font-serif text-4xl sm:text-5xl">Sua encomenda espera por você.</h2><p className="mt-5 max-w-lg leading-7 text-[#eadbce]">Não realizamos entregas. Escolha o melhor horário no checkout e retire seu pedido fresquinho na loja.</p></div><div className="grid gap-4">{settings.mapsUrl ? <a href={settings.mapsUrl} target="_blank" rel="noreferrer" className="group flex gap-4 rounded-2xl border border-white/10 bg-white/8 p-4 transition hover:-translate-y-0.5 hover:bg-white/12"><MapPin className="mt-1 shrink-0 text-[#e8bd80]" /><div className="flex-1"><strong>Endereço</strong><p className="mt-1 text-[#eadbce]">{settings.address}</p><span className="mt-2 inline-flex items-center gap-1 text-sm font-semibold text-[#f0c98f]">Abrir no mapa <ChevronRight className="size-4 transition group-hover:translate-x-0.5" /></span></div></a> : <div className="flex gap-4 rounded-2xl border border-white/10 bg-white/8 p-4"><MapPin className="mt-1 shrink-0 text-[#e8bd80]" /><div><strong>Endereço</strong><p className="mt-1 text-[#eadbce]">{settings.address}</p></div></div>}<div className="flex gap-4 rounded-2xl border border-white/10 bg-white/8 p-4"><Clock3 className="mt-1 shrink-0 text-[#e8bd80]" /><div><strong>Horário de retirada</strong><p className="mt-1 text-[#eadbce]">Das {settings.openTime} às {settings.closeTime}, conforme disponibilidade</p></div></div><div className="flex gap-4 rounded-2xl border border-white/10 bg-white/8 p-4"><AtSign className="mt-1 shrink-0 text-[#e8bd80]" /><div><strong>Fale com a gente</strong><p className="mt-1 text-[#eadbce]">{settings.phone} · {settings.instagram}</p></div></div></div></div></section>
      </main>
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="max-h-[94vh] overflow-y-auto rounded-3xl bg-[#fffaf4] sm:max-w-2xl">
      ...
      <form onSubmit={submit} className="grid gap-4 sm:grid-cols-2">
        ...
        <Button disabled={submitting} className={`h-13 text-base sm:col-span-2 ${primaryButton}`}>{submitting ? "Enviando pedido…" : "Confirmar pedido para retirada"}</Button>
      </form>
    </DialogContent>
  </Dialog>;
}
